...
3. 表示された**共有トークン**を控える（クライアントに同じ値を入れる）
...
| 再生成 | 「リモート」タブの「トークンを再生成」で新しい乱数に差し替えられます |
| 保存場所 | ホスト・クライアントとも `%APPDATA%\Argos\argos.db`（ローカル設定 DB）に保存。クラウドや外部へ送信されません |
| 漏洩時 | ホストで「トークンを再生成」→「設定を保存」し、接続する全クライアントの「リモートトークン」を新しい値に更新してください。旧トークンは直ちに無効になります |
