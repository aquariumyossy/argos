      ...
      {tab === "howto" ? (
        ...
        >
          ...
          <section>
            <h2>検索のコツ</h2>
            <p className="muted">
              チャットではなく、ショートカットで出る検索ポップアップの話です。Argos
              は日本語を単語に分けて探します。区切り方で「広く拾う」か「絞る」かが変わります。
            </p>
            <h3 className="howto-subhead">区切ると絞る</h3>
            <ul className="howto-tips">
              <li>
                文章をそのまま（区切りなし）→ 単語のどれか1つでも当たれば出ます（広く拾う）
              </li>
              <li>
                スペース・読点・改行で語を並べる → 大半の語が全部入っている文書だけ出ます（絞る）
              </li>
            </ul>
            <div className="howto-ex">
              <div>
                <code>解雇の有効性について</code>
                <span>「解雇」だけの文書も出る</span>
              </div>
              <div>
                <code>解雇 有効性</code>
                <span>両方ある文書だけ</span>
            ...
            <p className="muted">
              選択した文に読点（、）や改行があると、そこで切れて「絞る」側になります。定石は、一文を選んでショートカット →
              検索欄で不要な語を消し、残したい語をスペースで並べることです。
            </p>
            <h3 className="howto-subhead">書き方</h3>
            <ul className="howto-tips">
              <li>
                <code>&quot;解除通知&quot;</code> — 語が隣り合っていることを必須にします（半角の引用だけ。
                <code>「」</code> では効きません）
              </li>
              <li>
                <code>-慰謝料</code> / <code>-&quot;慰謝料請求&quot;</code> — その語・並びがある文書を除きます
              </li>
              <li>
                引用を1つでも付けると、囲った部分だけが必須になり、囲っていない語は順位用です。全部必須にしたいときは引用せずスペース区切りにしてください
              </li>
            </ul>
            <h3 className="howto-subhead">複合語と辞書</h3>
            <p className="muted">
              連続する名詞はひとまとまりにします。<code>裁判例</code>{" "}
              は「裁判」と「例」が隣にあるものとして探し、「例」だけの文書は出にくくなります。助詞を挟む用語（
              <code>弁済による代位</code>{" "}
              など）は、引用で囲むか、左の「辞書登録」に入れてください。登録語は以降の検索で自動的に隣接扱いです（
              <code>損害</code> だけでも探せます）。
            </p>
            <h3 className="howto-subhead">条文番号</h3>
            <ul className="howto-tips">
              <li>
                <code>第555条</code> や <code>民法555条</code> で、本文の{" "}
                <code>第五百五十五条</code> にも当たります
              </li>
              <li>
                数字の直後に条・項・号などが必要です。裸の <code>555</code>{" "}
                や日付・金額は条文とみなしません
              </li>
              <li>
                <code>&quot;第555条&quot;</code>{" "}
                と囲むと漢数字への展開をしません。条文検索では引用を外してください
              </li>
              <li>
                <code>民法 第555条</code>{" "}
                のように分けてよいです。条番号は必須、法令名は順位付けに使います
              </li>
            </ul>
            <h3 className="howto-subhead">結果と、うまくいかないとき</h3>
            <ul className="howto-tips">
              <li>
                一覧はファイル（またはメール）ごとに1件です。同じファイルの他の該当はプレビューの左右キーで辿ります
              </li>
              <li>
                条文ラベルや見出しで始まる段落が上に来やすいです。ファイル名だけの一致より、本文で語が近いヒットが優先されます
              </li>
              <li>
                増えすぎるときはスペース区切り・引用・<code>-語</code>・フォルダ／種別／時期
              </li>
              <li>
                当たらないときは、読点で切れていないか、条文に単位が付いているか、対象フォルダがインデックス済みかを見てください。画像のみの
                PDF は文字が取れていないと出ません
              </li>
            </ul>
          </section>
          ...
          <section>
            ...
            <ol className="howto-steps">
              ...
              <li>
                「チャット」から出典として送れます（長いメモは要約）。送った会話はそのノートを対象にします。回答の「ノートへ」でメモ末尾へすぐ追記できます。チャットからノートを直接作成・編集でき、差分はノートで確定または戻します
              </li>
            </ol>
          </section>
          ...
          <section>
            ...
            <ol className="howto-steps">
              ...
              <li>
                検索ヒットやノートの「チャット」から、送り先の会話を選べます。ノートから送るとそのノートが対象になり、回答は「ノートへ」でメモに戻せます。チャットからノートを直接作成・編集でき、差分はノートで確定または戻します。
              </li>
              <li>
